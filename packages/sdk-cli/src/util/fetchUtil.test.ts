...
describe('assertJSON()', () => {
  it.each(['application/text', 'image/png', undefined])(
    'rejects on Content-Type header of %s',
    (contentType) => {
      return expect(
  ...
  it.each(['application/json', 'application/json; charset=utf-8'])(
    'resolves on Content-Type header of %s',
    (contentType) => {
      return expect(
        fetchUtil.assertJSON()(
  ...
  it('accepts an alternative Content-Type to assert', () => {
    return expect(
      fetchUtil.assertJSON('application/vnd.document+json')(
        makeResponse({
          headers: { 'Content-Type': 'application/vnd.document+json' },
        }),
      ),
    ).resolves.toEqual({});
  });
...
describe('okOrElse', () => {
  ...
  it('resolves with the response if the response is ok', () => {
    ...
    return expect(fetchUtil.okOrElse(mockFailure)(response)).resolves.toBe(
      response,
    );
  });
