...
describe('maxDecodedSize', () => {
  ...
  it('is tested with correct test vectors', () =>
    expected.forEach((binary, encoded) => {
      expect(Buffer.alloc(binary).toString('base64').length).toBeLessThanOrEqual(encoded);
      expect(Buffer.alloc(binary + 1).toString('base64').length).toBeGreaterThan(encoded);
    }));
